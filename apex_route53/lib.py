...
def get_registrars():
    return DBSession.query(Registrar).order_by(Registrar.registrar).all()

def get_profile(id):
    return DBSession.query(Profile).filter(Profile.id == id).one()

def get_profiles():
    return DBSession.query(Profile).order_by(Profile.profile).all()
...
def get_profile_record(profile_id, record_id):
    return (DBSession.query(Profile_Record)
            ...
            .filter(Profile_Record.id == record_id)
            .one())

def get_profile_records(profile_id):
    return (DBSession.query(Profile_Record)
            .filter(Profile_Record.profile_id == profile_id)
            .order_by(Profile_Record.name)
            .all())
...
def get_ip(id):
    return DBSession.query(IP).filter(IP.id == id).one()

def get_ips():
    return DBSession.query(IP).order_by(IP.ip_address).all()

def get_ips_choices():
    ips = DBSession.query(IP.id, IP.ip_address, IP.note).order_by(
        IP.ip_address).all()
    return [(ip.id, '{0} ({1})'.format(ip.ip_address, ip.note)) for ip in ips]

def get_profiles_choices():
    return (DBSession.query(Profile.id, Profile.profile)
            .order_by(Profile.profile)
            .all())
